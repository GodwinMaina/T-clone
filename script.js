...
const getUsers = async ()=> {
    try {
        ...
        data.forEach(datas=>{
            ...
            options.value=datas.id;
            options.textContent=datas.name;
...
const getAperson = async (selectId) =>{
    try{
        ...
        console.log('No search person SORRY');
    }
}


//fetch a specific user on select by Id
function fetchUser ()  {
    const selectId = document.getElementById('selectUser').value;
...
const fetchPosts = async (UserPost)=>{
    try{

        const post = await fetch(`https://jsonplaceholder.typicode.com/posts?userId=${UserPost}`);
        const postdata = await post.json();
        // console.log(postdata);
        const postsContainer = document.getElementById('userall');
        postsContainer.innerHTML = ''; 

        postdata.forEach(postd=>{
                if(postd.userId==UserPost){
                    ...
                    <div class="img" >
                    <img id="profile1" class="abc" src="/user2.png" alt="" srcset=""> 
                    </div>    
                    ...
                    postsContainer.appendChild(postElement);
                        ...
                        // postParagraph.textContent = postd.body;
             ...
             // const posty = document.createElement('p')
             // posty.textContent=postd.body;
             // postbody.appendChild(posty);
          
    })
}

    catch{
        console.log('NO SEARCH POSTS');
    }
}
...
// // set slected user profile below navbar

// async function profileUser () {

//     try{

//         const userselect = document.querySelector('.profile').value;
//         const response = await fetch(`userUrl/${userselect}`);
...
//             // let selectedpic = document.createElement('div') 
//             let selectedperson= document.createElement('div')
//             let selectedusername= document.createElement('div')
//             let selectedwebsite= document.createElement('div')
//             let selectedphrase= document.createElement('div')
//             let selectedcity= document.createElement('div')

//             // selectedpic.setAttribute=userid.image;
//             selectedperson.textContent=userid.name;
//             selectedusername.textContent=userid.username;
//             selectedwebsite.textContent=userid.website;
//             selectedphrase.textContent=userid.catchphrase;
//             selectedcity.textContent=userid.city


//             // image.appendChild(selectedpic)

//             name.appendChild(selectedperson)
//             username.appendChild(selectedusername)
//             website.appendChild(selectedwebsite)
//             catchphrase.appendChild(selectedphrase)
//             city.appendChild(selectedcity)
...
//     catch{
//         console.log('No Search user exist...')
//     }
